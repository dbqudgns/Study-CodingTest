...
public class 퍼즐_조각_채우기 {
    ...
    public static int solution(int[][] game_board, int[][] table) {
        ...
        List<List<int[]>> boardList = new ArrayList<>(); // game_board의 빈칸 모양들
        List<List<int[]>> tableList = new ArrayList<>(); // table의 퍼즐 모양들
        ...
        // 테이블 퍼즐과 게임보드 빈칸을 비교해 맞는 조각 합 계산
    ...
    public static void bfs(int currentX, int currentY, boolean[][] visited, int[][] graph, int blockOrEmpty, List<List<int[]>> list) {
            ...
            for (int j = 0; j < boardLen; j++) {
                ...
                if (tablePoints.size() == boardPoints.size() && !visitedBoard[j]) {
                    ...
                    if (isRotate(boardPoints, tablePoints)) {
                        size += tablePoints.size(); // 맞으면 크기를 더한다.
                        visitedBoard[j] = true; // 해당 빈칸은 사용처리
                        break;
                    }
                }
            }
    ...
    public static boolean isRotate(List<int[]> board, List<int[]> table) {
        ...
        // board의 좌표들을 x(행) -> y(열) 순으로 정렬하자
        // x가 작은게 앞으로 x가 같다면 y가 작은게 앞으로 정렬


        // board를 정렬하는 이유 : table에서 board와 모양은 같은데 어떤 subList가 넘어오든 비교할 수 있도록 board의 모양을 하나의 표준 설계도로 만들어 두는 것 !!
        board.sort((o1, o2) -> {
            // 양수 : o1이 o2보다 뒤에 와야 한다.
            // 음수 : o1이 o2보다 앞에 와야 한다.
            return o1[0] > o2[0] ? 1 : o1[0] < o2[0] ? -1 : Integer.compare(o1[1], o2[1]);
        });


        // 최대 4번 회전 (0, 90, 180, 270)
        for (int i = 0; i < 4; i++) {
            // 현재 회전 상태의 table 정렬
            table.sort((o1, o2) -> {
                return o1[0] > o2[0] ? 1 : o1[0] < o2[0] ? -1 : Integer.compare(o1[1], o2[1]);
